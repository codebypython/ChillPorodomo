...
import { toggleFullscreen, lockOrientation, unlockOrientation } from '../utils/fullscreen'
...
function FocusPage() {
  ...
  const [soundTracks, setSoundTracks] = useState([{ id: Date.now(), selectedSound: null, type: 'single', volume: 1.0 }])
  ...
  const loadData = async () => {
    try {
      const [animationsData, soundsData, presetsData, sessionsData] = await Promise.all([
        getAnimations(),
        getSounds(),
            ...
            if (preset) {
              preset.soundIds.forEach(soundId => {
                ...
                if (sound) {
                  // Apply both sound's volume and track's volume
                  const finalVolume = (sound.volume || 1.0) * trackVolume
                  audioManager.playSound(`${track.id}_${soundId}`, sound.url, true, finalVolume)
                }
              })
            }
          }
  ...
  const handleAddSoundTrack = () => {
    setSoundTracks([...soundTracks, { id: Date.now(), selectedSound: null, type: 'single', volume: 1.0 }])
  }
  ...
  const handleSoundTrackChange = (id, value, type, volume) => {
    setSoundTracks(soundTracks.map(t => 
      t.id === id ? { ...t, selectedSound: value, type, volume: volume !== undefined ? volume : t.volume } : t
    ))
  }
  ...
  const backgroundImage = selectedBackgroundData && selectedBackgroundData.type !== 'video'
  ...
  return (
    <div 
      ...
      style={{
        ...
        backgroundColor: backgroundImage || backgroundVideo ? 'transparent' : '#f3f4f6'
      }}
    >
      {/* Video background */}
      {backgroundVideo && (
      ...
      {/* Content */}
      <div className={`relative z-10 min-h-screen flex flex-col transition-opacity duration-500 ${isUIHidden ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        {/* Header */}
          ...
          <div className="w-full max-w-4xl">
            ...
            <div className="text-center mb-6 sm:mb-12">
              ...
              <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mb-4 sm:mb-8 px-2">
                ...
                >
                  {isRunning ? <Pause size={20} className="sm:mr-2" /> : <Play size={20} className="sm:mr-2" />}
            ...
            <div className="bg-white bg-opacity-95 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-2xl space-y-4 sm:space-y-6 max-h-[60vh] sm:max-h-none overflow-y-auto">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <ChevronDown className="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none" size={20} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {soundTracks.map((track, index) => (
                    <div key={track.id} className="border border-gray-200 rounded-lg p-3 bg-gray-50">
                      <div className="flex items-start space-x-2 mb-3">
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-2">
                          <div className="relative">
                            <select
                              value={track.type}
                              onChange={(e) => handleSoundTrackChange(track.id, null, e.target.value)}
                          ...
                          <div className="relative">
                            <select
                              ...
                              onChange={(e) => handleSoundTrackChange(track.id, e.target.value || null, track.type)}
                              className="w-full px-3 py-2 pr-8 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-400 focus:border-transparent outline-none appearance-none bg-white text-sm"
                            >
                              <option value="">Chọn {track.type === 'single' ? 'sound' : 'preset'}</option>
                              {track.type === 'single' 
                                ? sounds.map((sound) => (
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <input
                          ...
                          onChange={(e) => handleSoundTrackChange(track.id, track.selectedSound, track.type, parseFloat(e.target.value))}
                          className="flex-1 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider"
                          style={{
                            background: `linear-gradient(to right, #a855f7 0%, #a855f7 ${(track.volume || 1.0) * 100}%, #e5e7eb ${(track.volume || 1.0) * 100}%, #e5e7eb 100%)`
                          }}
                        />
